...
export const sendOrder = async ({auth, mobile, basket,name,secondName,email}) => {
    if(auth) {
        const {data} = await $authHost({method:'POST', url: 'api/orders', data: {mobile, basket,name,secondName,email}})
        return data;
    } else {
        const {data} = await $host({method:'POST', url: 'api/orders', data: {mobile, basket,name,secondName,email}});
        return data;
    }
}
